...
const {uploadProductFile,uploadMultipleFiles,deleteMultipleFiles} = require("../../../assets/utils/uploadFiles");
...
route.post("/new-product-detail",async (req,res,next)=>{
    ...
    // }
     ...
     uploadMultipleFiles(req.files,__dirname+"/../../../assets/images/uploaded/",(uploadedImagePath,uploadMultipleImagePath)=>{

        AddProducts(req.body,uploadedImagePath,uploadMultipleImagePath).then(()=>{
...
route.get("/edit-specific-product/:id",async (req,res)=>{
    editSpecificProduct(req.params.id).then(data=>{
         res.json(JSON.stringify(data));
    }).catch(err=>{
        res.json(JSON.stringify(err));
    })
...
route.get("/load-specific-product/:id",async (req,res,next)=>{
    loadSpecificProduct(req.params.id).then(data=>{
        res.send(data);
    }).catch(err=>{
        next(err);
    });
...
const updateFile = (req,res,next,imgPath,multipleImagePath)=>{
    UpdateProducts(req.params.id,req.body,imgPath,multipleImagePath).then(data=>{
         res.json(JSON.stringify(data));
    }).catch(err=>{
        res.json(JSON.stringify(err));
    })
